...
public class ChatService {
    ...
    public ChatMessage sendMessage(String senderId, SendMessageRequest req)
        ...
        if (senderId.equals(convo.getAssignedStaffId()) && "ASSIGNED_TO_STAFF".equals(convo.getStatus())) {
            ...
            chatRepository.updateConversation(convo.getConvoId(), Map.of("status", "IN_PROGRESS"));
        }
    ...
    private void realtimeUseCase(){
        chatRepository.getFirestore().collection(ChatMessage.COLLECTION_NAME)
    ...
    public Conversation escalateConversationToStaff(String convoId) throws ExecutionException, InterruptedException {
        long now = System.currentTimeMillis();
        DocumentReference convoRef = chatRepository.getConversationReference(convoId);
        ...
        List<QueryDocumentSnapshot> checkIns = userRepository.findAttendanceByDateAndNoCheckOut(todayDate);
        ...
        if (candidates.isEmpty()) {
            List<QueryDocumentSnapshot> allStaffDocs = userRepository.findByRole("admin");
        ...
        if (!candidates.isEmpty()) {
            ...
            for (UserDTO staff : candidates) {
                int activeTickets = chatRepository.countActiveConversationsByStaff(staff.getUid());
    ...
    public void resolveConversation(String convoId) throws ExecutionException, InterruptedException {
        chatRepository.updateConversation(convoId, Map.of("status", "RESOLVED"));
    }

    public List<Conversation> getWaitingConversations() throws ExecutionException, InterruptedException {
        return chatRepository.getWaitingConversations();
    }
