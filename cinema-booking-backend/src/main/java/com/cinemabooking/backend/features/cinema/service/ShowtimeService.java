...
public class ShowtimeService {
    ...
    public List<ShowtimeDTO> getAllShowtimes() throws ExecutionException, InterruptedException {
        ...
        List<QueryDocumentSnapshot> documents = showtimeRepository.findAllOrderedByStartAt();
        List<ShowtimeDTO> showtimes = documents.stream()
                .map(this::mapToDTO)
                .filter(dto -> dto != null && !Boolean.TRUE.equals(dto.isDeleted()))
    ...
    public List<ShowtimeDTO> getShowtimesByMovieId(String movieId) throws ExecutionException, InterruptedException {
        ...
        List<ShowtimeDTO> showtimes = processQuerySnapshot(documents);
    ...
    private List<ShowtimeDTO> processQuerySnapshot(List<QueryDocumentSnapshot> documents) {
    ...
    public int seedShowtimes() throws ExecutionException, InterruptedException {
        ...
        List<QueryDocumentSnapshot> movieDocs = movieRepository.findAllLimit(100);
        ...
        for (Map<String, Object> movie : movies) {
            ...
            Map<String, Object> cinema = cinemas.get(cinemaIndex % cinemas.size());
            cinemaIndex++;
            String cinemaId = (String) cinema.get("cinemaId");

            List<Map<String, Object>> cinemaRooms = new ArrayList<>();
            ...
            for (int i = 0; i < 6; i++) {
                ...
                Map<String, Object> room = cinemaRooms.get(roomIdx);
                ...
                if (!existingDoc.exists()) {
                    Map<String, Object> showtimeData = new java.util.HashMap<>();
                    ...
                    showtimeRepository.save(showtimeId, showtimeData);
                    seedCount++;
                } else if ("AVAILABLE".equals(existingDoc.getString("status"))) {
                    showtimeRepository.updateStatus(showtimeId, "active");
                    seedCount++;
                }
            }
        }
    ...
    @Autowired
    @Lazy
    private com.cinemabooking.backend.features.movie.repository.MovieRepository movieRepository;
    ...
    public void cancelShowtime(String showtimeId) throws ExecutionException, InterruptedException {
        ...
        showtimeRepository.cancelShowtime(showtimeId, System.currentTimeMillis());
    ...
    public ShowtimeDTO updateShowtime(ShowtimeDTO showtime) throws ExecutionException, InterruptedException {
        return showtimeRepository.updateShowtimeInTransaction(showtime);
    }
}
