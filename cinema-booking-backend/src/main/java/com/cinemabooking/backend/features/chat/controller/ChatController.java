...
public class ChatController {
    ...
    public ResponseEntity<ApiResponse<Conversation>> initSupportConversation(
        ...
        if (convo == null) {
            ...
            if (convo.getStatus() == null) {
                ...
                conversationService.updateConversationStatus(convo.getConvoId(), "BOT_ONLY");
            }
        ...
        if (convo != null) {
            ...
            try {
                Conversation actualConvo = conversationService.getConversationById(convo.getConvoId());
                if (actualConvo != null) {
                    log.info("FIRESTORE_RAW_DOCUMENT={}", actualConvo.toString());
                } else {
    ...
    public ResponseEntity<ApiResponse<Void>> clearConversationMessages(
        ...
        if (user != null && ("staff".equalsIgnoreCase(user.getRole()) || "admin".equalsIgnoreCase(user.getRole()))) {
            if ("inactive".equalsIgnoreCase(user.getStatus()) || user.isDeleted()) {
                return ResponseEntity.status(403).body(
                        ApiResponse.<Void>builder()
                                .success(false)
